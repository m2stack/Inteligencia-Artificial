...
def constructBayesNet(gameState):
    ...
    # extraemos todas las variables observables y las añadimos a la lista de obsVars
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obsVar)

    # añadimos primero a edges las aristas de X positions e Y positions a Food house y Ghost house
    for start in X_POS_VAR, Y_POS_VAR:
        for end in FOOD_HOUSE_VAR, GHOST_HOUSE_VAR:
            edges.append([start, end])

    # ahora añadimos las aristas de la parte inferior de la red bayesiana (de cada 'house' a cada obsVar)
    for obsVar in obsVars:
        edges.append([GHOST_HOUSE_VAR, obsVar])
        edges.append([FOOD_HOUSE_VAR, obsVar])

    # para cada variableDomainsDict[var] añadimos los posibles valores que dicha 'var' pueda tener
    #-- posibles valores de X_POS_VAR
    variableDomainsDict[X_POS_VAR] = [FOOD_LEFT_VAL, GHOST_LEFT_VAL]
    #-- posibles valores de Y_POS_VAR
    variableDomainsDict[Y_POS_VAR] = [BOTH_TOP_VAL, BOTH_BOTTOM_VAL, LEFT_TOP_VAL, LEFT_BOTTOM_VAL]
    #-- posibles valores de FOOD_HOUSE_VAR
    variableDomainsDict[FOOD_HOUSE_VAR] = [TOP_LEFT_VAL, TOP_RIGHT_VAL, BOTTOM_LEFT_VAL, BOTTOM_RIGHT_VAL]
    #-- posibles valores de GHOST_HOUSE_VAR
    variableDomainsDict[GHOST_HOUSE_VAR] = [TOP_LEFT_VAL, TOP_RIGHT_VAL, BOTTOM_LEFT_VAL, BOTTOM_RIGHT_VAL]
   
    #-- posibles valores de cada obsVar
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            variableDomainsDict[obsVar] = [RED_OBS_VAL, BLUE_OBS_VAL, NO_OBS_VAL]

    # construimos un listado de todas las variables y creamos la red bayesiana
    variables = [X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, GHOST_HOUSE_VAR] + obsVars
    # var_list = [X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, GHOST_HOUSE_VAR, obsVars]
...
def constructStudentBayesNet():
    ...
    edges.append((DIFFICULTY_VAR, GRADE_VAR))
    edges.append((INTELLIGENCE_VAR, GRADE_VAR))
    edges.append((INTELLIGENCE_VAR, SAT_VAR))
    edges.append((GRADE_VAR,LETTER_VAR))

    # construimos la lista de variables
    variables = [DIFFICULTY_VAR, INTELLIGENCE_VAR, GRADE_VAR, SAT_VAR, LETTER_VAR]

    # construimos el dominio de cada variable
    ...
    # teniendo la red, añadimos las tablas de probabilidades, creando los factores y estableciendo las
    # distintas probabilidades 
    difficulty_factor = bn.Factor([DIFFICULTY_VAR], [], net.variableDomainsDict())
    difficulty_factor.setProbability({DIFFICULTY_VAR:EASY}, 0.6)
    difficulty_factor.setProbability({DIFFICULTY_VAR:HARD}, 0.4)

    intelligence_factor = bn.Factor([INTELLIGENCE_VAR], [], net.variableDomainsDict())
    intelligence_factor.setProbability({INTELLIGENCE_VAR:NOT_SMART}, 0.7)
    intelligence_factor.setProbability({INTELLIGENCE_VAR:SMART}, 0.3)

    sat_factor = bn.Factor([SAT_VAR], [INTELLIGENCE_VAR], net.variableDomainsDict())
    sat_factor.setProbability({SAT_VAR: LOW_SCORE, INTELLIGENCE_VAR:NOT_SMART}, 0.95)
    sat_factor.setProbability({SAT_VAR: LOW_SCORE, INTELLIGENCE_VAR:SMART}, 0.05)
    sat_factor.setProbability({SAT_VAR: HIGH_SCORE, INTELLIGENCE_VAR:NOT_SMART}, 0.2)
    sat_factor.setProbability({SAT_VAR: HIGH_SCORE, INTELLIGENCE_VAR:SMART}, 0.8)

    grade_factor = bn.Factor([GRADE_VAR], [DIFFICULTY_VAR, INTELLIGENCE_VAR], net.variableDomainsDict())
    grade_factor.setProbability({DIFFICULTY_VAR: EASY, INTELLIGENCE_VAR:NOT_SMART, GRADE_VAR:EXCELLENT}, 0.3)
    grade_factor.setProbability({DIFFICULTY_VAR: EASY, INTELLIGENCE_VAR:NOT_SMART, GRADE_VAR:GOOD}, 0.4)
    grade_factor.setProbability({DIFFICULTY_VAR: EASY, INTELLIGENCE_VAR:NOT_SMART, GRADE_VAR:AVERAGE}, 0.3)
    grade_factor.setProbability({DIFFICULTY_VAR: EASY, INTELLIGENCE_VAR:SMART, GRADE_VAR:EXCELLENT}, 0.9)
    grade_factor.setProbability({DIFFICULTY_VAR: EASY, INTELLIGENCE_VAR:SMART, GRADE_VAR:GOOD}, 0.08)
    grade_factor.setProbability({DIFFICULTY_VAR: EASY, INTELLIGENCE_VAR:SMART, GRADE_VAR:AVERAGE}, 0.02)
    grade_factor.setProbability({DIFFICULTY_VAR: HARD, INTELLIGENCE_VAR:NOT_SMART, GRADE_VAR:EXCELLENT}, 0.05)
    grade_factor.setProbability({DIFFICULTY_VAR: HARD, INTELLIGENCE_VAR:NOT_SMART, GRADE_VAR:GOOD}, 0.25)
    grade_factor.setProbability({DIFFICULTY_VAR: HARD, INTELLIGENCE_VAR:NOT_SMART, GRADE_VAR:AVERAGE}, 0.7)
    grade_factor.setProbability({DIFFICULTY_VAR: HARD, INTELLIGENCE_VAR:SMART, GRADE_VAR:EXCELLENT}, 0.5)
    grade_factor.setProbability({DIFFICULTY_VAR: HARD, INTELLIGENCE_VAR:SMART, GRADE_VAR:GOOD}, 0.3)
    grade_factor.setProbability({DIFFICULTY_VAR: HARD, INTELLIGENCE_VAR:SMART, GRADE_VAR:AVERAGE}, 0.2)

    letter_factor = bn.Factor([LETTER_VAR], [GRADE_VAR], net.variableDomainsDict())
    letter_factor.setProbability({GRADE_VAR: EXCELLENT, LETTER_VAR:WEAK_RECOMM_LETTER}, 0.1)
    letter_factor.setProbability({GRADE_VAR: EXCELLENT, LETTER_VAR:STRONG_RECOMM_LETTER}, 0.9)
    letter_factor.setProbability({GRADE_VAR: GOOD, LETTER_VAR:WEAK_RECOMM_LETTER}, 0.4)
    letter_factor.setProbability({GRADE_VAR: GOOD, LETTER_VAR:STRONG_RECOMM_LETTER}, 0.6)
    letter_factor.setProbability({GRADE_VAR: AVERAGE, LETTER_VAR:WEAK_RECOMM_LETTER}, 0.99)
    letter_factor.setProbability({GRADE_VAR: AVERAGE, LETTER_VAR:STRONG_RECOMM_LETTER}, 0.01)

    net.setCPT(DIFFICULTY_VAR, difficulty_factor)
    net.setCPT(INTELLIGENCE_VAR, intelligence_factor)
    net.setCPT(GRADE_VAR, grade_factor)
    net.setCPT(SAT_VAR, sat_factor)
    net.setCPT(LETTER_VAR, letter_factor)
...
def inferenceOnStudentBayesNet(bayes_net):
    "*** MODIFICATION ***"
    # variable que queremos consultar = LETTER_VAR
    queryVariables = [LETTER_VAR]
    # variables con valores que queremos definir = INTELLIGENCE_VAR:SMART , DIFFICULTY_VAR:HARD
    evidenceDict = {INTELLIGENCE_VAR: SMART, DIFFICULTY_VAR: HARD}

    # inferencia
    inf_enum = inference.inferenceByEnumeration(bayes_net, queryVariables, evidenceDict)
