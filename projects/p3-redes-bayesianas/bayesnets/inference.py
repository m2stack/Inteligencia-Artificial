...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # tomamos todos los factores de los que conocemos las variables de prueba (evidenceDict)
        factor_list = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # creamos una lista para los factores descartados
        discard = []

        # join all factors by variable
        for var in eliminationOrder:
            factor_list, joined_factor = joinFactorsByVariable(factor_list, var)

            if len(joined_factor.unconditionedVariables()) <= 1:
                discard.append(joined_factor)
            else:
                elimination = eliminate(joined_factor, var)
                factor_list.append(elimination)

        print("discard = ", discard)
        # unimos ahora todos los componentes extraidos a factor_list 
        new_join = joinFactors(factor_list)

        # normalizamos y devolvemos el factor
        normal_factor = normalize(new_join)
